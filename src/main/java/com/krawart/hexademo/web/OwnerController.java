...
class OwnerController {
  ...
  @PostMapping
  ResponseEntity<OwnerDTO> addOwner(
    @RequestBody @Valid AddOwnerCommand command
  ) {
    var result = ownerService.add(command);
    return ResponseEntity
      .created(URI.create("/api/v1/owners/" + result.getId()))
      .build();
  }

  @PutMapping("/{id}")
  ResponseEntity<OwnerDTO> updateOwner(
    @RequestBody @Valid UpdateOwnerCommand command,
