...
class MedicineController {
  ...
  @PostMapping
  ResponseEntity<MedicineDTO> addMedicine(
    @RequestBody @Valid AddMedicineCommand command
  ) {
    var result = medicineService.add(command);
    return ResponseEntity
      .created(URI.create("/api/v1/medicines/" + result.getId()))
      .build();
  }

  @PutMapping("/{id}")
  ResponseEntity<MedicineDTO> updateMedicine(
    @RequestBody @Valid UpdateMedicineCommand command,
