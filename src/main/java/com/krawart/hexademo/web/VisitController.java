...
class VisitController {
  ...
  @PostMapping
  ResponseEntity<VisitDTO> addVisit(
    @RequestBody @Valid AddVisitCommand command
  ) {
    var result = visitService.add(command);
    return ResponseEntity
      .created(URI.create("/api/v1/visits/" + result.getId()))
      .build();
  }

  @PutMapping("/{id}")
  ResponseEntity<VisitDTO> updateVisit(
    @RequestBody @Valid UpdateVisitCommand command,
